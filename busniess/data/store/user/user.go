...
func (s Store) Create(ctx context.Context, nu NewUser, now time.Time) (User, error) {
	// validate the input value by validate package: validate.Struct
	// in Struct call, it uses val := reflect.ValueOf(nu), and val.Type() get the NewUser
	// And we already set validate tag in the struct definition
...
func (s Store) Query(ctx context.Context, pageNumber int, rowsPerPage int) ([]User, error) {
	...
	// the variable(:offset & :rows_per_page) passed into the query below
	data := struct {
...
// QueryByID gets the specified user from the database by user_id.
...
func (s Store) QueryByEmail(ctx context.Context, claims auth.Claims, email string) (User, error) {
	...
	if err := database.NamedQueryStruct(ctx, s.log, s.db, q, data, &usr); err != nil {
		...
		return User{}, fmt.Errorf("selecting by email[%q]: %w", email, err)
	}
...
// Authenticate finds a user by their email and verfifies their password.
...
func (s Store) Authenticate(ctx context.Context, now time.Time, email, password string) (auth.Claims, error) {
	...
	if err := database.NamedQueryStruct(ctx, s.log, s.db, q, data, &usr); err != nil {
		...
		return auth.Claims{}, fmt.Errorf("selecting by email[%q]: %w", email, err)
	}
	...
	claims := auth.Claims{
		StandardClaims: jwt.StandardClaims{ // TODO: replace with jwt.RegisteredClaims
			Issuer:    "serive project",
			Subject:   usr.ID,
