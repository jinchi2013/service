// packgage database: provides support for access the database
package database
...
var (
	...
	ErrForbidden             = errors.New("attemped action is not allowed")
)
...
// Open knows how to open a database connnection based on the configuration
...
func StatusCheck(ctx context.Context, db *sqlx.DB) error {
	...
	for attemps := 1; ; attemps++ {
		...
		// Context timeout, would stop the loop
		if ctx.Err() != nil {
			return ctx.Err()
		}
	}
...
// NamedQuerySlice is a helper function for executing queries
// that return a collection of data to be unmarshaled into a slice
func NamedQuerySlice(
	...
	// check to make sure dest is a pointer to a slice
...
// NamedQueryStruct is a helper function for executing queries
// that return a single value to be unmarshalled into a struct type.
